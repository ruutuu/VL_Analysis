...
#data<-read_sheet('1xhHTDo_6FMxfg6bTpghM5xXYdiw3nQzA',sheet="allstudies")
...
df_undefdeno<-subset(df,!df$eff_arm_icure_num>0)
#4 studies with unclear denominator
...
df_all<-subset(df1,!df1$me_hiv_yn=="Yes")#124 with HIV yes and unknown  AND 2 with no data. 
length(unique(df_all$pub_id))
df_hiv<-subset(df1,df1$me_hiv_yn=="Yes")
length(unique(df_hiv$pub_id)) #14 with HIV
#124 left
...
df_final1<-df_final[,c(1,11,44,36,49,40,35,12,13,27:29,31,45,47,46,48,51,50,2:10,14:26,30,32,33,34,37,38,39,41:43)]
